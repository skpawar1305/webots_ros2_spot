...
class SpotDriver:
    ...
    def __talker(self, motors_target_pos):
        self.params = [self.sliders[i].get() for i in range(NUMBER_OF_JOINTS)]
        for idx, motor in enumerate(self.motors):
            motor.setPosition(motors_target_pos[idx] - self.motors_initial_pos[idx] + self.params[idx])
    ...
    def spot_inverse_control(self):
        ...
        target = [
            joint_angles[0][0] - 0.1, joint_angles[0][1] - 1, joint_angles[0][2] + 1.5,
            joint_angles[1][0] + 0.1, joint_angles[1][1] - 1, joint_angles[1][2] + 1.5,
            joint_angles[2][0] - 0.1, joint_angles[2][1] - 1, joint_angles[2][2] + 1.5,
            joint_angles[3][0] + 0.1, joint_angles[3][1] - 1, joint_angles[3][2] + 1.5,
            ]
